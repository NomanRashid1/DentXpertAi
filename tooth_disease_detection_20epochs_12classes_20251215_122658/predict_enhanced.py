...
class ToothDiseasePredictor:
    ...
    def __init__(self, model_path: str, gemini_api_key: str = None):
    ...
    def process_detections(self, results, image_path: str) -> List[Dict]:
        """Process YOLO detections and extract all parameters"""
        all_detections = []
        ...
        for r in results:
            ...
            for box in r.boxes:
                ...
                # Classify disease based on confidence and tooth position
                ...
                # Extract precise polygon contour
                ...
                detection = {
                    ...
                    "recommendations": disease_info.recommendations[:3],  # Top 3
        ...
        # Deduplicate: Keep only the highest confidence detection for each tooth number
        tooth_best = {}
        for det in all_detections:
            tooth_num = det['tooth_number']
            if tooth_num not in tooth_best or det['confidence'] > tooth_best[tooth_num]['confidence']:
                tooth_best[tooth_num] = det
        
        # Convert back to list and sort by tooth number
        detections = list(tooth_best.values())
    ...
    def create_annotated_image(self, image_path: str, detections: List[Dict], unique_id: str) -> str:
        ...
        # Rainbow colors from reference - assigned by tooth number
        rainbow_colors = [
            '#00FF00', # Green
            '#FFFF00', # Yellow
            '#00FFFF', # Cyan
        ...
        # 1. Draw all tooth polygons on the overlay
        for idx, detection in enumerate(detections):
            # Get color
            tooth_number = detection.get('tooth_number', idx)
            color_idx = tooth_number % len(rainbow_colors)
            color_hex = rainbow_colors[color_idx]
            color = tuple(int(color_hex.lstrip('#')[i:i+2], 16) for i in (0, 2, 4))
            
            # Get polygon
            polygon_points = detection.get('polygon', [])
            
            if polygon_points and len(polygon_points) > 2:
                # 50% alpha (128) for semi-transparent look
                overlay_color = color + (128,)
                overlay_draw.polygon(polygon_points, fill=overlay_color)
                
                # Border on overlay (or main image? Overlay keeps it simple)
                border_color = color + (255,)
                overlay_draw.polygon(polygon_points, outline=border_color, width=3)

        # 2. Composite the overlay onto the main image
        annotated_image = annotated_image.convert('RGBA')
        annotated_image = Image.alpha_composite(annotated_image, overlay)
        annotated_image = annotated_image.convert('RGB')
        
        # 3. Draw labels on the final composite (so they are on top)
        draw = ImageDraw.Draw(annotated_image)
        ...
        for idx, detection in enumerate(detections):
            ...
            tooth_number = detection.get('tooth_number', idx)
            color_idx = tooth_number % len(rainbow_colors)
            color_hex = rainbow_colors[color_idx] # Need hex for border
            ...
            while attempt < max_attempts:
                ...
                if attempt == 0: label_y = y2 + 5
                elif attempt == 1: label_x = x2 - text_width
                elif attempt == 2: label_y = y1 + (y2 - y1) // 2; label_x = x2 + 5
                elif attempt == 3: label_x = x1 - text_width - 5
                else:
                    label_y += random.randint(-20, 20)
                    label_x += random.randint(-10, 10)
                
                label_x = max(0, min(label_x, annotated_image.width - text_width))
                label_y = max(0, min(label_y, annotated_image.height - text_height))
                attempt += 1
            ...
            # Draw label
            padding = 4
            bg_bbox = [label_x - padding, label_y - padding, label_x + text_width + padding, label_y + text_height + padding]
