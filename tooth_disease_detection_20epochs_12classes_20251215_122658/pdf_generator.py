...
class PDFReportGenerator:
    ...
    def _create_summary_section(self, results: dict):
        ...
        # Summary table
        summary_data = [
            ['<b>Total Teeth Detected</b>', str(summary.get('total_teeth', 0))],
        ]
        
        # Disease distribution
        disease_dist = summary.get('disease_distribution', {})
        if disease_dist:
            summary_data.append(['<b>Disease Distribution</b>', ''])
        ...
        if severity_dist:
            summary_data.append(['<b>Severity Distribution</b>', ''])
    ...
    def _create_detailed_findings(self, results: dict):
        ...
        # Create table data - header row (bold applied via TableStyle)
        table_data = [
            ['Tooth #', 'Disease', 'Severity', 'Area', 'Confidence', 'Urgency']
        ]
...
def generate_pdf_report(prediction_results: dict, output_filename: str = None) -> str:
    """
